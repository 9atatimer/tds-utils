...
- [ ] Task GI1: **NATS request-reply reads** (`gadmin.issues.list`, `gadmin.issues.get.<n>`). Aggregator publishes events but has no subscribe/respond handlers; clients always read direct from GH. Extend the NATS helper at `gadmin/admin/issue-aggregator.mjs:216-240` with subscription handlers and teach `cmdIssueList` / `cmdIssueView` / `cmdIssueNext` in both `.mjs` peers to try NATS first with GH fallback. Goal 3 in the design doc.
- [ ] Task GI2: **NATS-first `--wait-tx`** via a `gadmin.tx.wait` request-reply subject. `maybeWaitTx` at `gadmin/admin/github-gitapi.mjs:845` calls only `pollAppliedReceipt`, so even on the laptop a waiter pays the 3s GH-poll interval. Add a `gadmin.tx.wait` subscriber on the aggregator that resolves on `gadmin.events.applied`/`rejected`; fall through to existing poll on NATS unreachable. Goal 4.
- [ ] Task GI3: **`gh webhook forward` as primary ingress** with poll as fallback. Aggregator polls only every `--interval` seconds; its own header at `gadmin/admin/issue-aggregator.mjs:22-24` flags webhook forwarding as deferred. Spawn `gh webhook forward` as a child process, HMAC-verify payloads (see design doc's Security section) before queueing. Goal 6.
