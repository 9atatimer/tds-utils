...
- [x] Task LMDE16: **Fix the shell startup order LMDE14 broke.** Keep `.zshenv` visibility for agent shells, but move PATH *ordering* into a single `tds_path_apply` function re-invoked from `.zprofile` (after `path_helper`) and `.zshrc` (after the runtime managers), and evict `nvm.sh`/`pyenv init`/`brew shellenv` from `.zshenv`. Contract pinned by `test/smoketest_shell_env.sh`. Issue #176.
...
- [ ] Extend `test/smoketest_lmde_acquire/config.sh` stubs: optional 5th
...
- [ ] `14_skills_data_package_floats_and_links.sh` -- Given no pins, When
...
- [ ] `15_skills_version_pin_honored.sh` -- Given SKILLS_VERSION="0.1.0"
  while stub latest is 9.9.9, Then skills installs at 0.1.0 and the other
  packages float (mirror of scenario 02).
- [ ] Run `./test/smoketest_lmde_acquire/run_all.sh`: 13 existing GREEN,
...
- [ ] `acquire_pkg_table` row: `skills @nine-at-a-time-media/skills - SKILLS_VERSION`
  (bin column `-` = data-only sentinel; document in the column comment).
- [ ] `acquire_one`: data-package branch -- presence check is
...
- [ ] `bin/lmde` usage text mentions skills.
- [ ] Run `./test/smoketest_lmde_acquire/run_all.sh`: all 15 GREEN.

**Commit Point:** `feat(acquire): install the skills data package (GREEN)`

### Step 3: Doc folds

- [ ] `sandbox/pins.env`: add `SKILLS_VERSION="UNSET"` (float is the design
  intent) and rewrite the stale "CLAI_VERSION is the content-stamp gate"
  header per Revision 1.
- [ ] `lmde/LMDE.md`: acquire section -- three packages + skills; retire the
  "NOT separately acquired" passage.
- [ ] `sandbox/README.md` / `lmde/acquire-pins.env.example`: mention
...
- **Consumer-owned config cannot live inside a distributed skill bundle (the LOCAL.md misstep)**: `skills/github-workflow/` shipped a `LOCAL.md.example` each consuming repo was told to copy to `LOCAL.md`, fill in, and commit; the shared `SKILL.md` read that sibling for owner/repo slug, default branch, topology, and branch prefixes. It could never work: `clai provision` materializes skills as **symlinks into one shared tree** (`~/.cache/clai/template-tools/skills/<name>`), so "this skill's directory" is the same directory for every repo on the machine and the last writer wins for all of them. The example file's own instructions were self-contradictory -- "copy it to LOCAL.md in this same directory" and "commit LOCAL.md to your repo" cannot both hold when that directory is a cache clone of another repo. The general rule: **a skill directory is a unit of distribution** -- versioned, refreshed, overwritten by the provisioner, exactly like `node_modules` -- so consumer-owned configuration must never live inside it. The replacement is to **derive** identity at runtime (`gh repo view --json nameWithOwner/defaultBranchRef/isFork,parent` -- authoritative, cannot go stale like a hand-copied `Default Branch: master`) and to read the non-derivable residue (branch prefixes, cadence limits, required check names) from the consuming repo's `AGENT.md`/`CLAUDE.md`, which is already the canonical home for repo policy. Retired in `Nine-At-A-Time-Media/template-tools` before any repo adopted it. Generalizable test before adding any config file: **who owns this directory, and what happens when the owner next rewrites it?**
- **GUI-Spawned Agents and the Crippled PATH**: A fundamental divide exists between CLI tools (like `gemini-cli`) and GUI-spawned coding agents (like Antigravity). CLI tools inherit the user's interactive, login shell environment (fully hydrated by `.zprofile` and `.zshrc`). However, GUI apps spawn background agents using non-interactive, non-login shells (e.g., `zsh -c "..."`). By `zsh` convention, these shells **only** source `~/.zshenv`. If the user's PATH setup is confined to `.zshrc` or `.zprofile`, the agent wakes up with a barren `launchd` environment and crashes when attempting to invoke standard tools (`gh`, `npm`, `gadmin`, `git-mirror`). The robust fix is migrating all pure environment definitions into `.zshenv`, instantly curing the OS-level path malady.
  - **AMENDMENT (#176): `.zshenv` owns PATH membership, but it cannot own PATH order.** The above is right about *visibility* and wrong about *precedence*. macOS `/etc/zprofile` runs `/usr/libexec/path_helper` **after** `~/.zshenv`; it rebuilds `PATH` with `/etc/paths` + `/etc/paths.d` first and appends the inherited `PATH` behind them, demoting everything `.zshenv` prepended. `$HOMEBREW_PREFIX/bin` sank below `/bin`, so `#!/usr/bin/env bash` resolved Apple Bash 3.2, which lacks `readarray` -- that aborted the `clai` pre-hook. Order must be re-asserted in `.zprofile`, after `path_helper`. Two corollaries: state the ordering **once**, as a function called from all three files, or they drift; and make it **idempotent**, because `.zshrc` execs tmux and the whole startup sequence runs again. A third lesson came free: cost. Sourcing `nvm.sh` in `.zshenv` costs ~456ms on *every* agent-spawned `zsh -c` (682ms startup, now 17ms). A runtime manager's PATH contribution can usually be derived statically -- nvm's default node bin is one read of `$NVM_DIR/alias/default` -- without paying for the manager itself.
