...
- [x] CI for the dist suite: GitHub Actions workflow running the four dist
      ...
      (`.github/workflows/dist-ci.yml` + `test/syntax_sweep.sh`; five suites
      now, incl. tds-status; macOS leg asserts /bin/bash IS 3.2 and pins
      `/bin/bash` for sweep + suites so Homebrew bash 5 cannot mask 3.2.)
...
- [ ] Task LMDE7: **Events: design reconcile.** Revise the observability sections of `docs/design/LMDE.DESIGN.md` -- refine the "Log Aggregation" Non-Goal (structured OTLP events in scope; raw terminal logs stay `log-hoarder`'s), add the Loki events architecture, note Tempo as the deferred traces leg.
