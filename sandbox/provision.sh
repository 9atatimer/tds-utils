...
# GitHub Packages npm reads require the classic read:packages scope; fine-grained
...
write_npmrc() {
  ...
  if [ -z "$token" ]; then
    note "GH_PAT_NAATM_PACKAGES_RO unset -- need a classic read:packages PAT to install clai from GitHub Packages"
    return 1
  fi
