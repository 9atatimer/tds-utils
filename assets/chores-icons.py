...
Regenerate only when changing the artwork:
    ...
    uv run --with pillow python assets/chores-icons.py
"""
...
def draw_quiet() -> Image.Image:
    ...
    d.line([(0.82 * s, 0.10 * s), (0.42 * s, 0.56 * s)], fill=BLACK, width=int(0.11 * s))
    # head: a fat trapezoid at the end of the handle
    d.polygon(
        [(0.46 * s, 0.50 * s), (0.30 * s, 0.62 * s), (0.10 * s, 0.92 * s),
         (0.52 * s, 0.92 * s), (0.58 * s, 0.62 * s)],
        fill=BLACK,
    )
    # bristle gaps cut through to transparent, so the head reads as a broom
    for x in (0.24, 0.36, 0.48):
        d.line([(x * s, 0.74 * s), ((x - 0.03) * s, 0.92 * s)], fill=CLEAR, width=int(0.035 * s))
...
def draw_alert() -> Image.Image:
    ...
    d = ImageDraw.Draw(img)
    s = MASTER
    d.polygon([(0.50 * s, 0.06 * s), (0.96 * s, 0.90 * s), (0.04 * s, 0.90 * s)], fill=RED)
    d.rounded_rectangle([0.44 * s, 0.32 * s, 0.56 * s, 0.64 * s], radius=int(0.04 * s), fill=CLEAR)
